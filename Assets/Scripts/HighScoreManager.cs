...
/// 
/// this is a singleton class.  to access these functions, use HighScoreManager.instance object.
/// eg: HighScoreManager.instance.SaveHighScore("meh",1232);
...
public class HighScoreManager : MonoBehaviour {
	...
	public void SaveHighScore (string name, int score) {
		...
		// Retrieve saved scores for comparison
		...
		if (HighScores.Count == 0) {            
			...
			for (i = 1; i <= HighScores.Count && i <= LeaderboardLength; i++) {
				...
				// Add lower score (comment if only want high scores)
	...
	public void ClearLeaderBoard () {
		int scoresCount = GetHighScores().Count;
